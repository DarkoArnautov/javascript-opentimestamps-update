...
class Timestamp {
  ...
  shrink() {
    ...
    const allAttestations = this.allAttestations();
    if (allAttestations.size === 0) {
        throw new Error("No attestations available.");
    } else if (allAttestations.size === 1) {
        return Array.from(allAttestations.values())[0];
    } else if (this.ops.size === 0) {
        throw new Error("No operations available.");
    }

    // For >1 attestations: Search for the first BitcoinBlockHeaderAttestation
    let minAttestation = null;

    for (const [op, timestamp] of this.ops.entries()) {
        for (const attestation of timestamp.getAttestations()) {
            if (attestation instanceof Notary.BitcoinBlockHeaderAttestation) {
                if (!minAttestation) {
                    minAttestation = attestation;
                } else if (
                    minAttestation instanceof Notary.BitcoinBlockHeaderAttestation &&
                    attestation instanceof Notary.BitcoinBlockHeaderAttestation &&
                    minAttestation.height > attestation.height
                ) {
                    minAttestation = attestation;
                }
            }
    ...
    // If no BitcoinBlockHeaderAttestation is found, return the first attestation
    if (!minAttestation) {
        return Array.from(allAttestations.values())[0];
    }

    // Remove attestations that are not the minAttestation
    let shrinked = false;

    for (const [op, timestamp] of this.ops.entries()) {
        const attestations = timestamp.getAttestations();

        if (attestations.size > 0 && attestations.has(minAttestation) && !shrinked) {
            timestamp.shrink();
            shrinked = true;
        } else {
            this.ops.delete(op);
