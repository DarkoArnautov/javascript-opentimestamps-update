...
const upgradeCommand = program
  ...
  const shrinkCommand = program
  .command('shrink [FILE_OTS]')
  .alias('h')
  .description('shrink timestamps have only earlest bitcoin block')
  .action((file) => {
    isExecuted = true
    if (!file) {
      console.log(shrinkCommand.helpInformation())
      return
    }
    shrink(file)
  })
...
async function shrink(argsFileOts) {
  try {
    const ots = await Utils.readFilePromise(argsFileOts, null);
    ...
    // Shrink
    await OpenTimestamps.shrink(detachedOts)
    
    await fs.promises.writeFile(argsFileOts + 'upgraded.ots.bak', Buffer.from(ots), 'binary', err => {
      if (err) {
        return console.log(err)
      }
      console.log('The file .bak was saved!')
    ...
    // Write to file
    await fs.promises.writeFile(argsFileOts + '.shrinked.ots', Buffer.from(detachedOts.serializeToBytes()), 'binary', err => {
      if (err) {
        return console.log(err)
      }
    })
