...
#### Shrink
When a proof is completed with at least one bitcoin attestation, information regarding other paths are redundant. The oldest bitcoin attestation (the lower bitcoin block height) is the only relevant information, in this case we can shrink all the redundant information.

```shell
$ ots-cli.js shrink example/hello-world.txt.ots
```
...
async function getShrinkedOTSObject (detached) {
  await OpenTimestamps.shrink(detached);
     const shrinkedFileOts = detached.serializeToBytes();

     return shrinkedFileOts;
}
```
